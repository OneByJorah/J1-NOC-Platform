...
async def list_tickets(request: Request):
    ...
    async with httpx.AsyncClient(timeout=5) as client:
        try:
            r = await client.get(
                f"{OSTICKET_BASE}/api/http.php/tickets.json", headers=HEADERS, params=params
            )
...
async def get_ticket(ticket_id: str):
    async with httpx.AsyncClient(timeout=5) as client:
        try:
            r = await client.get(
                f"{OSTICKET_BASE}/api/http.php/tickets/{ticket_id}.json", headers=HEADERS
            )
            return JSONResponse(r.json(), status_code=r.status_code)
        except Exception:
            pass
...
async def create_ticket(request: Request):
    ...
    async with httpx.AsyncClient(timeout=20) as client:
        r = await client.post(
            f"{OSTICKET_BASE}/api/http.php/tickets.json", headers=HEADERS, json=body
        )
    return JSONResponse(r.json(), status_code=r.status_code)
